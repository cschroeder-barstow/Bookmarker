```
██████╗  ██████╗  ██████╗ ██╗  ██╗███╗   ███╗ █████╗ ██╗  ██╗███████╗██████╗ 
██╔══██╗██╔═══██╗██╔═══██╗██║ ██╔╝████╗ ████║██╔══██╗██║ ██╔╝██╔════╝██╔══██╗
██████╔╝██║   ██║██║   ██║█████╔╝ ██╔████╔██║███████║█████╔╝ █████╗  ██████╔╝
██╔══██╗██║   ██║██║   ██║██╔═██╗ ██║╚██╔╝██║██╔══██║██╔═██╗ ██╔══╝  ██╔══██╗
██████╔╝╚██████╔╝╚██████╔╝██║  ██╗██║ ╚═╝ ██║██║  ██║██║  ██╗███████╗██║  ██║
╚═════╝  ╚═════╝  ╚═════╝ ╚═╝  ╚═╝╚═╝     ╚═╝╚═╝  ╚═╝╚═╝  ╚═╝╚══════╝╚═╝  ╚═╝
```
![Static Badge](https://img.shields.io/badge/current_build-v1.1.1-red)

Bookmarker is a free, open-source, lightweight bookmark manager.


**How to use:**
<br>
To use the bookmark manager, click the add bookmark button and enter the website details
example: 
```
Title:
e.g. GitHub
URL:
https://example.com or app://link
Image URL optional:
https://example.com/icon.png
```
( Note: you can also add apps with app:// )



Update Log:

See full update log + more info at: https://cschroeder-barstow.github.io/Bookmarker/docs.html

- 08/24/26 - v1.0.0 public release
- 08/29/26 - v1.0.1 bug fixes and ui rework
- 09/14/26 - v1.1.0 auto image feature
- 09/15/26 - v1.1.1 bug fixes with ui scaling
  
Made by galaxydev - 08/24/26
